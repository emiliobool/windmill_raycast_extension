...
export default function ListWorkspacesCommand(props: LaunchProps) {
    ...
    return (
        ...
        >
            {workspaces.length === 0 ? (
                <List.EmptyView title="No Workspaces found" description={searchText !== "" ? "Press Enter to add your workspace" : "Type the name of your workspace to create one"}
                    actions={<ActionPanel>
                        <Action.Push title="Add Workspace" icon={Icon.Plus} shortcut={{ modifiers: ["cmd"], key: "n" }} target={<WorkspaceForm name={searchText} onCreate={fetchWorkspaces} onDelete={fetchWorkspaces} />} />
                    </ActionPanel>}
                />
            ) : (
